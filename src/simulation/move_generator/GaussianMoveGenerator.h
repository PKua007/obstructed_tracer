...
public:
...
#if CUDA_HOST_COMPILATION
    /**
     * @brief A `__host__` constructor, which initializes the generators using seeds generated by byte generator seeded
     ...
     */
    CUDA_HOST GaussianMoveGenerator(float sigma, float integrationStep, unsigned int seed);

#else // CUDA_DEVICE_COMPILATION
    /**
     * @brief A `__device__` constructor, which initializes the generators using seeds generated by byte generator
     * seeded with @a seed parameter
     *
     ...
     */
    CUDA_DEV GaussianMoveGenerator(float sigma, float integrationStep, unsigned int seed, size_t numberOfTrajectories);
